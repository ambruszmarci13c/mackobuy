...
$kategoriak_roviditese = [
    "elektronikai eszközök" => "elekt",
    "egészségügyi termékek" => "egi",
    "háztartási kellékek" => "hazt",
    "játékok" => "jatek",
    "kerti eszközök" => "kert",
    "könyvek" => "konyv"
];
...
<body>
    <div class="container my-5">
        ...
        <?php if ($kategoria_szures): ?>
            ...
            <div class="row">
                <?php if (!empty($szurt_termekek)): ?>
                    <?php 
                    $szamlalo = 1; // Képek számlálója
                    $rovidites = $kategoriak_roviditese[$kategoria_szures] ?? ''; // Rövidítés meghatározása
                    foreach ($szurt_termekek as $termek): 
                    ?>
                        <div class="col-md-4">
                            <div class="card mb-4">
                                <!-- Kép elérési útvonal helyes összeállítása -->
                                <img src="kepek/<?= $rovidites ?>/<?= $rovidites . $szamlalo ?>.jpg" class="card-img-top" alt="<?= htmlspecialchars($termek['tnev']) ?>">
                                <div class="card-body">
        ...
        <div class="row">
            ...
            <?php 
            $osszes_szamlalo = [];
            foreach ($osszes_termek as $termek): 
                $kat = $termek['kategoria'];
                $rovidites = $kategoriak_roviditese[$kat] ?? '';
                if (!isset($osszes_szamlalo[$rovidites])) {
                    $osszes_szamlalo[$rovidites] = 1;
                }
            ?>
                <div class="col-md-4">
                    <div class="card mb-4">
                        <!-- Kép elérési útvonal helyes összeállítása -->
                        <img src="kepek/<?= $rovidites ?>/<?= $rovidites . $osszes_szamlalo[$rovidites] ?>.jpg" class="card-img-top" alt="<?= htmlspecialchars($termek['tnev']) ?>">
                        <div class="card-body">
                            <h5 class="card-title"><?= htmlspecialchars($termek['tnev']) ?></h5>
                            <p class="card-text"><?= htmlspecialchars($termek['leiras']) ?></p>
                            <p class="card-text"><strong>Ár:</strong> <?= htmlspecialchars($termek['ar']) ?> Ft</p>
