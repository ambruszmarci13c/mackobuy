...
$user_id = $_SESSION['user_id'];
$query = "SELECT t.ID, t.tnev, t.ar, t.kep, t.leiras, t.garancia
          FROM termekek t
                ...
                <div class="termek-kartya">
                    ...
                    <form method="POST">
                        ...
                        <button type="submit" class="eltavolitas-gomb">Eltávolítás</button>
                         <!-- Részletek gomb -->
                         <button type="button" class="reszletek-gomb" data-bs-toggle="modal" data-bs-target="#termekModal<?= $row['ID'] ?>">Részletek</button>

                    </form>
                </div>

                  <!-- Modal (a while cikluson belül!) -->
                  <div class="modal fade" id="termekModal<?= $row['ID'] ?>" tabindex="-1" aria-labelledby="termekModalLabel<?= $row['ID'] ?>" aria-hidden="true">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="termekModalLabel<?= $row['ID'] ?>"><?= htmlspecialchars($row['tnev']) ?></h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Bezárás"></button>
                            </div>
                            <div class="modal-body">
                                <div class="text-center">
                                    <img src="kepek/<?= htmlspecialchars($row['kep']) ?>" class="img-fluid mb-3" alt="<?= htmlspecialchars($row['tnev']) ?>">
                                    <p><?= nl2br(htmlspecialchars($row['leiras'])) ?></p>

                                    <?php if ($row['garancia']): ?>
                                        <p>Garancia: <?= htmlspecialchars($row['garancia']) ?> hónap</p>
                                    <?php endif; ?>

                                    <p><strong>Ár:</strong> <?= $formazott_ar ?></p>
                                </div>
                            </div>
