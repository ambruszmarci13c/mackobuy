...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['termek_id']) && !isset($_POST['vezeteknev'])) {
        ...
        if (empty($vezeteknev) || empty($keresztnev) || empty($email) || empty($telefonszam) || empty($cim) || empty($szallitas) || empty($fizetes)) {
            ...
            // 1. LÉPÉS: Termékek összértékének kiszámítása
            $vegosszeg = 0;
            ...
            // 2. LÉPÉS: Szállítási díj hozzáadása
            $szallitasi_dij = $szallitas_dijak[$szallitas] ?? 0;
            $vegosszeg += $szallitasi_dij;

            // 3. LÉPÉS: Utánvételi díj hozzáadása (ha van)
            if ($fizetes === 'utanvet') {
                $vegosszeg += $utanvet_dij;
            }

           // 4. LÉPÉS: Kuponkedvezmény alkalmazása (ha van)
            $kupon_kedvezmeny = 0;
            $kuponok = [
            ...
            if (array_key_exists($kupon_kod, $kuponok)) {
                $kupon_kedvezmeny = $kuponok[$kupon_kod];
                $vegosszeg -= $kupon_kedvezmeny;
                $vegosszeg = max(0, $vegosszeg); // Nem lehet negatív a végösszeg
            }

            $statusz = "feldolgozás alatt";

            // Felhasználó ID lekérése (ha be van jelentkezve)
            $felhasznalo_id = $_SESSION['user_id'] ?? null;

            // Rendelés dátumának formázása
            $rendeles_datum = date("Y-m-d");

            // Adatbázisba mentés
            if ($felhasznalo_id) {
                // Ellenőrizd, hogy a felhasználó ID nem 0
                if ($felhasznalo_id > 0) {

                    // **Ellenőrizd, hogy a felhasználó ID létezik a felhasznalok táblában!**
                    $felhasznalo = adatokLekeres("SELECT ID FROM felhasznalok WHERE ID = ?", [$felhasznalo_id]);
                    if ($felhasznalo) {
                        // Az ID mezőt NE add meg az INSERT utasításban!
                        $muvelet = "INSERT INTO rendelesek (FID, vegosszeg, rendeles_datum, szallitasi_dij, statusz) VALUES (?, ?, ?, ?, ?)";
                        $parameterek = [
                            (int)$felhasznalo_id,
                            (float)$vegosszeg,
                            $rendeles_datum,
                            (float)$szallitasi_dij,
                            $statusz
                        ];
                        ...
                        if ($eredmeny === "Sikeres művelet!") {
                            ...
                            $hiba_uzenet = "Hiba történt a megrendelés rögzítésekor! " . $eredmeny;
                        }
                    } else {
                        $hiba_uzenet = "Érvénytelen felhasználói azonosító. Kérjük, jelentkezzen be újra!";
                    }
                ...
                $hiba_uzenet = "A rendelés leadásához be kell jelentkeznie!";
            }
    }
...
<body>
    ...
    <div id="sikeresMegrendelesAblak" class="overlay <?php echo $megrendeles_sikeres ? '' : 'hidden'; ?>">
        <div class="popup">
            ...
            <button id="sikeresrendelesgomb" onclick="document.getElementById('sikeresMegrendelesAblak').classList.add('hidden')" class="btn btn-primary">OK</button>
        </div>
    </div>
    ...
    <div class="container my-5">
        ...
        <?php if (!empty($_SESSION['kosar'])): ?>
            ...
            <form method="POST" action="kosar.php">
                    ...
                    <label class="d-block">
                        <div id="posta" class="mb-3 p-3 border rounded">
                ...
                <div id="gls" class="mb-3 p-3 border rounded">
                    <label class="d-block">
                ...
                <label class="d-block">
                    <div id="kartya" class="mb-3 p-3 border rounded">
                ...
                <div id="utanvetel" class="mb-3 p-3 border rounded">
                    <label class="d-block">
                ...
                <div id="kuponInputContainer" class="mt-3 d-none">
                    <input type="text" name="kupon_kod" id="kuponKod" class="form-control w-50 d-inline" placeholder="Írd be a kuponkódot">
                ...
                <div class="text-center mt-4">
                    ...
                    <span id="kiszallitasspan" class="text-muted fs-6">
                        <img src="kepek/ketmunkanap.png" alt="asd" style="height: 35px;">
                        Várhatóan 2 munkanapon belül kiszállításra kerül.
                    </span>
                </div>
                    <!-- Hibaüzenet (ha van) -->
